...
fn main() {
    ...
    for i in 0..m {
        let val = seg.query(b[i], b[i] + 1, 0, 0, seg.nn);
        seg.modify(b[i], b[i] + 1, -val, 0, 0, seg.nn);

        let full = val / n as i64;
        let rest = (val % n as i64) as usize;
        seg.modify(0, n, full, 0, 0, seg.nn);

        // [s, t)
        let s = b[i] + 1;
        let t = b[i] + 1 + rest;
        if s < n && t <= n {
            seg.modify(s, t, 1, 0, 0, seg.nn);
        } else if s < n && t > n {
            seg.modify(s, n, 1, 0, 0, seg.nn);
            seg.modify(0, t - n, 1, 0, 0, seg.nn);
        } else {
            seg.modify(s - n, t - n, 1, 0, 0, seg.nn);
        }
    }

    let mut ans = vec![];
    for i in 0..n {
        ans.push(seg.query(i, i + 1, 0, 0, seg.nn));
    }
    println!("{}", join(&ans, " "));
}
