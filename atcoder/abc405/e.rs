...
fn main() {
    ...
    // Reference: https://codeforces.com/blog/entry/142744?#comment-1274095
    // 1. Once the position of apple and bananas is determined, grapes and oranges are also determined.
    // 2. If the last position of apple is at one-based index i (a <= i <= n - g - b),
    //    then there are comb(i - 1, a - 1) * comb(n - i, b) ways to put.
    //    comb(i - 1, a - 1) is the way to put apples.
    //    comb(n - i, b) is the way to put bananas.

    let inp = readv::<usize>();
    let (a, o, b, g) = (inp[0], inp[1], inp[2], inp[3]);
    let n = a + o + b + g;
    let tool = CombTool::new(n);

    let mut ans = Mint(0);
    for i in a..=(n - g - b) {
        ans += tool.comb(i - 1, a - 1) * tool.comb(n - i, b);
    }
    println!("{ans}");
}
